...
def plot_frequencies_text_color(freq_data, title, xlabel, color, num_elements):
    max_width = 340  # Maximum width of the bar in characters
    # ANSI color codes
...
def calculate_e():
    ...
    return str(mp.e).replace('.', '')

def calculate_phi():
    print("Calculating φ (Golden Ratio)...")
    return str(mp.phi).replace('.', '')
...
if __name__ == '__main__':
    ...
    pi_str = pool.apply_async(calculate_pi).get()
    e_str = pool.apply_async(calculate_e).get()
    g_str = pool.apply_async(calculate_phi).get()

    # Close the pool and wait for all processes to finish
    pool.close()
    pool.join()
    
    print("Calculating Pi, e, and φ completed.")
